...
export default function Members() {
  ...
  const badge = (ok:boolean): React.CSSProperties => ({
    ...
    color: ok ? "#a7f3d0" : "#e5e7eb",
    background: ok ? "rgba(16,185,129,.18)" : "rgba(255,255,255,.06)",
  ...
  return (
    <div style={{ padding: "0 14px 24px" }}>
      ...
      {/* Filters */}
      <div style={{display:"flex", gap:10, flexWrap:"wrap", justifyContent:"space-between", margin:"0 14px 12px"}}>
        <div style={{display:"flex", gap:10, flexWrap:"wrap"}}>
          <input
            ...
            style={input}
          />
          <select value={status} onChange={(e)=>setStatus(e.target.value as any)} style={inputSel}>
            <option value="all">All</option>
            <option value="approved">Approved</option>
            <option value="pending">Pending</option>
          </select>
          <select value={year} onChange={(e)=>setYear(e.target.value)} style={inputSel}>
            {["All","1st","2nd","3rd","4th"].map(y=><option key={y} value={y}>{y}</option>)}
          </select>
        </div>
      ...
      <div style={grid(260, 16)}>
        {loading
          ...
          : filtered.map((m) => (
              <article key={m.id} style={card(true, "dark")} className="hoverable">
                <div style={{ color: "#fff", fontWeight: 900, fontSize: 18 }}>{m.full_name}</div>
                <div style={{ color: "rgba(255,255,255,.78)", marginTop: 6, fontSize: 13 }}>
                  Year: {m.year_of_study}
                </div>
                <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:12}}>
                  <span style={badge(m.is_member)}>{m.is_member ? "Approved" : "Pending"}</span>
